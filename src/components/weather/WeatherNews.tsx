  ...
  if (!newsArticles || newsArticles.length === 0) {
    return (
      <Card className="mt-6 shadow-lg">
        ...
        <CardContent>
          <p className="text-muted-foreground">No recent weather news found.</p>
        </CardContent>
      </Card>
    );
  }
  ...
  return (
    <Card className="mt-6 shadow-lg">
      <CardHeader>
        ...
        <CardDescription>Latest weather-related headlines.</CardDescription>
      </CardHeader>
      <CardContent>
        <div className="space-y-6">
          {newsArticles.map((article) => (
            <div key={article.id} className="flex flex-col sm:flex-row gap-4 p-4 border rounded-lg hover:shadow-md transition-shadow">
              {article.imageUrl && (
                <div className="w-full sm:w-1/3 md:w-1/4 aspect-video relative overflow-hidden rounded-md">
                  <Image
