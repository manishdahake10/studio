...
export async function fetchWeatherData(city: string): Promise<WeatherAPIResponse> {
  ...
  try {
      ...
      try {
        ...
        if (errorData && errorData.message) {
          ...
          if (String(errorData.cod) === "401") {
             errorMessage = `Invalid OpenWeatherMap API key for current weather: ${errorData.message}. Please ensure your key in .env is correct and for OpenWeatherMap.`;
      ...
      try {
        ...
        if (errorData && errorData.message) {
          errorMessage = `Forecast: ${errorData.message} (code ${errorData.cod})`;
           if (String(errorData.cod) === "401") {
             errorMessage = `Invalid OpenWeatherMap API key for forecast: ${errorData.message}. Please ensure your key in .env is correct and for OpenWeatherMap.`;
          } else if (String(errorData.cod) === "404") {
    ...
    const location = {
      ...
      tz_id: `Etc/GMT${timezoneOffsetSeconds >= 0 ? '+' : '-'}${Math.abs(timezoneOffsetSeconds/3600)}`,
    ...
    if (forecastData.list && forecastData.list.length > 0) {
      ...
      Object.keys(dailyDataAgg).sort().slice(0, 7).forEach(dateStr => {
        ...
        const dayForecast: DayForecast = {
          ...
          daily_will_it_rain: dayEntries.some(e => e.pop && e.pop > 0.3) ? 1 : 0,
        ...
        const hourForecasts: HourForecast[] = dayEntries.map(item => ({
          ...
          wind_mph: Math.round(item.wind.speed * 2.23694),
